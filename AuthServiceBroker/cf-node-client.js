    ...
    request(loginAPICallReqOptions, function (err, result) {
        if (err) {
            ...
            if (result.statusCode && result.statusCode == 200) {
                ...
                deleteAppFinalCallback(null, JSON.parse(result.body));
            }
            else {
...
CfNodeClient.prototype.pushApp = function (options, callback) {
    ...
    var zip = function (loginResults,callback) {
    ...
    var push = function (app, callback) {
        ...
        Apps.add(appOptions).then(function (result) {
            ...
            var routeOptions = {
                "domain_guid": config["domain_guid"],
                "space_guid": config["space_guid"],
    ...
    async.waterfall([loginAPICall, zip, push], function (err, result) {
        if (err) {
        ...
        var getDatacallback = function (err, result) {
            if (err) {
                ...
                if (index == -1) {
                    ...
                    resultObj.accessToken = 'bearer ' + loginResult.access_token;//loginResult;
